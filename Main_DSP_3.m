...
%                 reward_abs = 1/abs(log(predictedEntropy(count)));
                predictedReward(i) = reward_rel*sign(preEntropy - predictedEntropy(count));
                % learing from prediction
                agent = agent.dpLearning(1,1,i,predictedReward(i)); % there is only one state (1)
            end
        ...
        agent = agent.dpLearning(1,1,idx,reward(k)); % there is only one state (1)
