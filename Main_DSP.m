    ...
    for k = 1 : N,
        ...
        if (k==1) || (k<30) || (rem(k,1/(fs_ctr*T))==0),
            for i = 1 : length(para),
                ...
                if sum(xkk1~=xkk1_RL) || sum(sum(Skk~=Skk_RL))
                    disp('###### xkk ######');
                    pause
                end
...
%                 
%                 %%% Calculate measurement covariance: R
%                 R = computeR_FI(c, fc, lmd, b, r_snr0, M, xkk1_RL(1)-H);
%                 Rsqrt       = mysqrt(R);
%                 %%% Approximate cost function: J(\theta)
%                 Skk   = ComputeSkk(xkk1_RL,Skk1_RL,Rsqrt);% + p_ratio*Skk_p;
%                 entropy(i)= trace(Lambda*(Skk*Skk'));
%             end
%             
%             [min_entropy,idx] = min(entropy) % find the minimum of J
%             pause
%             lmd_RL    = para(1,idx);
%             b_RL      = para(2,idx);
%             wf_lmd_RL(expt, k) = lmd_RL;
%             wf_b_RL(expt, k) = b_RL;
%             wf_idx_RL(expt,k) = idx;
%         end
%         %%% Calculate measurement covariance R using new waveform
%         R_RL = computeR_FI(c, fc, lmd_RL, b_RL, r_snr0, M, xkk1_RL(1)-H);
%         Rsqrt_RL       = mysqrt(R_RL);
%         z_RL     = z_idl + Rsqrt_RL* mmm;
%         [xkk_RL,Skk_RL] = Update(xkk1_RL,Skk1_RL,z_RL,Rsqrt_RL);
        %%%================================================================
