...
window = 10;
numPlanningTrials = 2;   % must NOT be greater than number of actions
alphaState = .1 * [1 1 1]; % learnign factor conrresponding to each state
...
%     preEntropy3 = det(Lambda*(Skk_RL3*Skk_RL2')); % initial entropy
    ...
    for k = 1 : N,
        ...
        for j = 1:length(rewardP)
            tempAlpha = agent.alpha;
            agent.alpha = alphaState(j);
            agent = agent.dpLearning(1,1,idx,reward(j)); % there is only one state (1)
            agent.alpha = tempAlpha;
        end
...
%                 agent3 = agent3.dpLearning(1,1,i,predictedReward3(i)); % there is only one state (1)
                ...
                rewardRelP3 = abs((abs(eig(preP3) - eig(predictedP3) )) );
                rewardP3 = rewardRelP3.*sign( eig(preP3) - eig(predictedP3) );  
                
                % learing from prediction
                for j = 1:length(rewardP3)
                    tempAlpha = agent3.alpha;
                    agent3.alpha = alphaState(j);
        ...
        rewardRelP3 = abs((abs(eig(preP3) - eig(newP3))));
        rewardP3 = rewardRelP3.*sign( eig(preP3) - eig(newP3) );
        
        % learning by Q-learning method
        for j = 1:length(rewardP3)
            tempAlpha = agent3.alpha;
            agent3.alpha = alphaState(j);
            agent3 = agent3.dpLearning(1,1,idx,reward3(j)); % there is only one state (1)
            agent3.alpha = tempAlpha;
        end
